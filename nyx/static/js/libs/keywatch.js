 ...
 *   data-keybind-icon="💾"            ícone para o modal de atalhos
...
class Keywatch {
    ...
    _injectStyles() {
        ...
        style.textContent = `
            ...
            #keywatch-modal {
                --kw-bg:           #1a1b26;
                --kw-bg-row-alt:   #1f2133;
                --kw-surface:      #24253a;
                --kw-border:       #2e3050;
                --kw-accent:       #7c6af7;
                --kw-accent-soft:  rgba(124,106,247,.15);
                --kw-text:         #c8cde8;
                --kw-text-dim:     #6b7094;
                --kw-text-header:  #a0a8cc;
                --kw-badge-bg:     #2e3050;
                --kw-badge-border: #4a4f7a;
                --kw-input-bg:     #13141f;
                --kw-shadow:       0 24px 64px rgba(0,0,0,.6);
                --kw-radius:       12px;
                --kw-radius-sm:    6px;
                --kw-font:         'Segoe UI', system-ui, -apple-system, sans-serif;
                --kw-font-mono:    'Cascadia Code', 'Fira Code', 'Consolas', monospace;
    ...
    _createModal() {
        ...
        header.innerHTML = `
            <span class="kw-icon">${this.shortcutMaplistIcon || '⌨'}</span>
                ...
                for (const scope in this.handlers[type][ctx]) {
                    for (const handler of this.handlers[type][ctx][scope]) {
                        ...
                        tr.innerHTML = `
                            ...
                            <td>${handler.icon ? `<span style="margin-right:6px">${handler.icon}</span>` : ''}${handler.desc || '<span style="opacity:.4">—</span>'}</td>
                            <td><span class="kw-key" style="font-size:10px">${handler.context}</span></td>
                        `;
    ...
    scanBindings(root = document) {
        ...
        elements.forEach(el => {
            ...
            const options = {
                ...
                display:        el.dataset.keybindDisplay        !== 'false',
                element:        document,
            };
